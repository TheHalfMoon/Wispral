#!/usr/bin/env python3
"""Execute B2R09 sherpa-onnx-compact under frozen ATTEMPT-002 C0."""
...
class DecodeError(ValueError):
    """Raised when B2R09 authority or execution invariants fail closed."""
...
def validate_authority() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    require(readiness.get("completed_recovery_tasks") == [f"B2R{i:02d}" for i in range(1, 9)], "B2R09 predecessor ledger drift")
    ...
    require(replacement.get("attempt_id") == ATTEMPT_ID and replacement.get("frozen") is True, "ATTEMPT-002 authority drift")
...
def verify_operational_qualification() -> dict[str, Any]:
    ...
    require(len(matches) == 1, "exact sherpa compact smoke evidence missing")
    row = matches[0]
    require(row.get("status") == "SMOKE_PASS", "sherpa compact operational smoke did not pass")
    require(row.get("runtime_revision") == EXPECTED_RUNTIME_REVISION, "sherpa runtime revision drift")
    require(row.get("runtime") == {"distribution": "sherpa-onnx", "version": EXPECTED_RUNTIME_VERSION}, "sherpa runtime package drift")
...
def execute(model_root: Path, preprocessed_root: Path, runtime_source: Path) -> dict[str, Any]:
    ...
    require(config.get("id") == CANDIDATE_ID and config.get("tier") == "COMPACT", "candidate cell 5 identity drift")
    ...
    require(expected_names == {
        "encoder-epoch-99-avg-1-chunk-16-left-128.int8.onnx",
        "decoder-epoch-99-avg-1-chunk-16-left-128.int8.onnx",
        "joiner-epoch-99-avg-1-chunk-16-left-128.int8.onnx",
        "bpe.model", "tokens.txt",
    }, "sherpa compact artifact set drift")
    ...
    source_revision = subprocess.run(
        ["git", "-C", str(runtime_source), "rev-parse", "HEAD"], check=True, text=True,
        stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=10,
    ).stdout.strip()
    require(source_revision == EXPECTED_RUNTIME_REVISION, "pinned sherpa source revision drift")
    source_tree = subprocess.run(
        ["git", "-C", str(runtime_source), "rev-parse", "HEAD^{tree}"], check=True, text=True,
        stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=10,
    ).stdout.strip()
    ...
    encoder = operational_smoke.find_artifact(model_root, "encoder-epoch-99-avg-1-chunk-16-left-128.int8.onnx")
    decoder = operational_smoke.find_artifact(model_root, "decoder-epoch-99-avg-1-chunk-16-left-128.int8.onnx")
    joiner = operational_smoke.find_artifact(model_root, "joiner-epoch-99-avg-1-chunk-16-left-128.int8.onnx")
    ...
    recognizer = sherpa_onnx.OnlineRecognizer.from_transducer(
        ...
        num_threads=4, provider="cpu", sample_rate=SAMPLE_RATE, feature_dim=80,
        decoding_method="greedy_search", max_active_paths=4,
        lm="", lm_scale=0.1, lodr_fst="", lodr_scale=-0.1,
        hotwords_file="", hotwords_score=1.5, modeling_unit="", bpe_vocab="",
        blank_penalty=0.0,
    )
...
def main() -> int:
    ...
    print(f"B2R09_INPUTS={evidence['execution']['input_count']}")
