#!/usr/bin/env python3
"""Execute bounded B2E01 C0 decoding for the frozen moonshine-compact cell."""
...
SCHEMA = "000b2-public-b2e01-decode-v1"
TASK = "B2E01"
ATTEMPT_ID = "000B2-PUBLIC-ATTEMPT-001"
CANDIDATE_ID = "moonshine-compact"
EXPECTED_AUTHORITY_BASE = "6607b1b1a13daebe1c267f82e3295be9b3bdea32"
...
class DecodeError(ValueError):
    """Raised when B2E01 authority or execution invariants fail closed."""
...
def validate_authority() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    require(isinstance(next_action, str) and next_action.startswith("Execute B2E01 only:"), "B2E01 is not the sole canonical next action")
    require("moonshine-compact" in next_action, "B2E01 candidate identity missing from canonical next action")
    require("Do not begin B2E02" in next_action, "B2E02 successor boundary missing from canonical next action")

    current = CURRENT_PATH.read_text(encoding="utf-8")
    tasks = TASKS_PATH.read_text(encoding="utf-8")
    require("current bounded execution unit `B2E01`" in current, "CURRENT does not own B2E01 frontier")
    require("B2E01 is the sole current bounded execution unit" in current, "CURRENT B2E01 sole-unit authority missing")
    ...
    require(isinstance(candidate_ids, list) and candidate_ids and candidate_ids[0] == CANDIDATE_ID, "candidate cell 1 drift")
...
def runtime_provenance() -> dict[str, Any]:
    ...
    return {
        ...
        "container_image": os.environ.get("B2E01_CONTAINER_IMAGE", ""),
...
def execute(work_dir: Path, preprocessed_root: Path) -> dict[str, Any]:
    ...
    evidence: dict[str, Any] = {
        ...
        "claim_guards": {
            ...
            "completed_through": "B2E01_EXECUTION_ONLY",
...
def main() -> int:
    ...
    try:
        ...
        print("B2E01_EXECUTION=CAPTURED")
        ...
        print(f"B2E01_EXECUTION=FAIL: {error}", file=sys.stderr)
        return 1
