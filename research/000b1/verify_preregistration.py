...
def validator_module():
    return load_module("validate_attempt_manifest.py", "b2_validator")


def frozen_contract_module():
    return load_module("verify_frozen_contracts.py", "frozen_contract_verifier")


def entity_gate_module():
    return load_module("verify_entity_annotation_gate.py", "entity_annotation_gate")


def verify_candidates():
    # The semantic verifier pins the complete family/cell allowlist, model identities,
    # runtime/source provenance, selection rule, payload sizes, artifact identities,
    # pinned/pending digests, and tier contract. Keep this call in the top-level
    # verifier so a permissive local shape check cannot accidentally become authority.
    frozen_contract_module().verify_registry()
    ...
    whisper = next(f for f in m["families"] if f["family"] == "whisper.cpp")
...
def verify_schemas():
    for name in (
        ...
        schema = load(HERE / "schemas" / name)
        if schema.get("$schema") != "https://json-schema.org/draft/2020-12/schema" or not schema.get("$id"):
            fail(f"invalid schema header: {name}")
    # Header checks are not sufficient authority. Pin semantic required fields,
    # constants, tier bounds, candidate IDs, and frozen-methodology constraints.
    frozen_contract_module().verify_schema_semantics()
...
def verify_entity_annotation_semantics():
    # Execute regression cases for start/end ordering, transcript bounds,
    # exact span text, and duplicate entity IDs as part of this top-level gate.
    if entity_gate_module().main() != 0:
        fail("entity annotation semantic regression failed")
...
def verify_adversarial_review():
    ...
    if "`b2_ready: no`" not in text:
        fail("adversarial review must preserve exact B2_READY: NO authority")
...
def main() -> int:
    try:
        ...
        verify_entity_annotation_semantics()
