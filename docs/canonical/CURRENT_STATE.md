...
- B2P06 authorized as the sole current bounded execution unit;
- candidate revalidation is complete; no comparative public-corpus decoding has started, B2P06 preprocessing capture remains unresolved until its own execution unit, and no production STT is selected.
...
Execute and canonically qualify `B2P06` only: capture attempt-bound FFmpeg `9.0.1` preprocessing identity, configuration, and execution evidence against the frozen B2P04 public-human source membership and canonical B2P05 candidate identities. B2P06 must bind the exact FFmpeg release/source identity and exact preprocessing command/configuration to the current attempt and must preserve source-to-preprocessed byte identity evidence without candidate-specific decoding or output inspection. Preserve `candidate_decoding_started=false`, `primary_decoding_started=false`, the historical private lane as `BLOCKED_EXTERNAL`, `HUMAN_DEVELOPER_SPEECH_ACCURACY_EVIDENCE=ABSENT`, and the closed product-code gate. B2P07 remains non-authorized until B2P06 is canonically qualified, post-merge verified, and reconciled.
